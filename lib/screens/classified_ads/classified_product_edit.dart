...
import 'package:geebay/data_model/category.dart';
...
class _ClassifiedProductEditState extends State<ClassifiedProductEdit> {
  ...
  update() async {
    ...
    if (response.result != null && response.result!) {
      ToastComponent.showDialog(response.message, gravity: ToastGravity.CENTER);

      Navigator.pop(context);
    } else {
      dynamic errorMessages = response.message;
      if (errorMessages.runtimeType == String) {
        ToastComponent.showDialog(errorMessages, gravity: ToastGravity.CENTER);
      } else {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          elevation: 0,
          title: Text(
            LangText(context).local.add_new_classified_product_ucf,
            style: TextStyle(
                fontSize: 16,
